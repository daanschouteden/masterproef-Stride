...
{
        string first="", second="", third="", household="", k12school="", college="", workplace="", primary="", secondary="", cluster="";
	...
	std::mt19937 gen(rd());
        ...
        if (numSteps != 0U) {
                ...
                for (unsigned int i = 0; i < numSteps; i++) {
                        // We are AtStart: no steps have taken yet, so signal AtStart.
                        if (m_sim->GetCalendar()->GetSimulationDay() == 0) {
                                m_sim->TimeStep(first, second, third, household, k12school, college, workplace, primary, secondary, cluster, false, gen);
                                Notify(Id::Stepped);
                        }
