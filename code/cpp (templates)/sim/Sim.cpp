...
//				isHouseholdClusteringAllowed << " " << endl;
		...
		*/
		...
		#pragma omp parallel num_threads(m_num_threads)
		{
			...
			for (size_t i = 1; i < poolSys.RefPools(typ).size(); i++) { // NOLINT
				...
				m_infector_default(poolSys.RefPools(typ)[i], m_contact_profiles[typ], m_transmission_profile,
				...
				if (typ == ContactType::Id::SecondaryCommunity) {
					auto duration = duration_cast<microseconds>(stop - start);
					times[poolSys.RefPools(typ)[i].size()] += duration.count();
					counts[poolSys.RefPools(typ)[i].size()]++;
        		}
				*/
			}
		}
	...
	/*
	for (auto typ : ContactType::IdList) {
		if (!(typ == ContactType::Id::PrimaryCommunity || typ == ContactType::Id::SecondaryCommunity)) {
				continue;
		}
			// Infector updates individuals for contacts & transmission within each pool.
			// Skip pools with id = 0, because it means Not Applicable.
			...
			for (size_t i = 1; i < poolSys.RefPools(typ).size(); i++) { // NOLINT
					m_infector_tracing(poolSys.RefPools(typ)[i], m_contact_profiles[typ], m_transmission_profile,
								m_handlers[0], simDay, eventLogger,
								workplace_distancing_factor,
								community_distancing_factor,
								school_distancing_factor,
								intergeneration_distancing_factor,m_cnt_reduction_intergeneration_cutoff,
