...
private:
        ...
        // Sort the susceptibles by age intervals going from young to old
        std::vector<unsigned int> SortSusceptiblesByAgeIntervalsYO(unsigned int num_cases);

        // Sort the infectious by age intervals going from young to old
        std::vector<unsigned int> SortInfectiousByAgeIntervalsYO(unsigned int num_cases);

        // Sort by age intervals going from young to old
        std::vector<unsigned int> SortMembersByAgeIntervalsYO();

        // Sort by age intervals going from old to young
        std::vector<unsigned int> SortMembersByAgeIntervalsOY();

        // Count the amounts in the age intervals without sorting
        std::vector<unsigned int> CountTotalIntervals();

        unsigned int PeopleInPool();

        const static int workplace_interval_ages[3];
        const static int community_interval_ages[9];
        const static int school_interval_ages[8];

        const static int workplace_interval_ages2[3];
        const static int community_interval_ages2[9];
