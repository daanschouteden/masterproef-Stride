...
const int ContactPool::school_interval_ages[] = {1, 3, 6, 12, 18, 23, 65, 120};
...
std::vector<unsigned int> ContactPool::SortSusceptiblesByAgeIntervalsYO(unsigned int num_cases)
{
        ...
        else if (m_pool_type == ContactType::Id::K12School || m_pool_type == ContactType::Id::College) {
                amount_intervals = sizeof(school_interval_ages)/sizeof(school_interval_ages[0]);
                age_intervals = school_interval_ages;
        }
        else
        {
                std::cerr << "Error" << endl;
